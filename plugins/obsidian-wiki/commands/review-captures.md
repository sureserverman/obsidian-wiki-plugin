...
Use the `review-captures` skill to list `session-capture` entries the
...
pick which to extract via `import-session`.
...
to `import-session`, which writes the actual extract and the
